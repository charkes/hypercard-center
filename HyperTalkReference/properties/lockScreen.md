...
` set `[`the`]<code> lockScreen to [ph:trueOrFalse]

</code>The `lockScreen` property returns or sets whether HyperCard updates the screen when you go to another card. You can use `lockScreen` to prevent the user from seeing cards as a handler goes to them.
